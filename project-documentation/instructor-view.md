...
|---|---|
| `site/index.html` | *For the module owners* — the link to the LRD, and the links to each week's presenter deck with `?instructor` already on |
| `site/week-NN-deck.html` | The instructor notes panel and the Notes button of the presenter deck, and the notes on its printed pages. Generated by `project-documentation/slides/build_deck.py`, which copies the toggle from `index.html` at build time |
